...
void ADC_init(void){
	...
	//Turning MODER for ADC for PC1 analog mode
	GPIOC->MODER |= (1<<2);
	GPIOC->MODER |=(1<<3);
...
{//BUZZER
	...
	GPIOC->MODER &= ~(1<< 18);  // This clears both bits moder 9 bcz pc9 so 18,19th bits
	GPIOC->MODER |= (1<< 19);
	...
	// low for 0 to 6 /for timer 3 alternate function pC6 bcz af2 0010 (27,26,25,24) PG 286
			GPIOC->AFR[0] &= ~(1 << 24);
			GPIOC->AFR[0] &= ~(1 << 26);
			GPIOC->AFR[0] &= ~(1 << 27);
			GPIOC->AFR[0] |= (1 << 25);
